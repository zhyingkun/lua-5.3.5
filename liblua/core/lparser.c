  ...
  if (!testnext(ls, what)) {
    if (where == ls->linenumber)
      ...
      luaX_syntaxerror(
          ls,
          luaO_pushfstring(
              ls->L, "%s expected (to close %s at line %d)", luaX_token2str(ls, what), luaX_token2str(ls, who), where));
    }
  }
}
...
static void closegoto(LexState* ls, int g, Labeldesc* label) {
  ...
  if (gt->nactvar < label->nactvar) {
    ...
    const char* msg = luaO_pushfstring(
        ls->L, "<goto %s> at line %d jumps into the scope of local '%s'", getstr(gt->name), gt->line, getstr(vname));
    semerror(ls, msg);
  }
