...
static void updateWindowMode(_GLFWwindow* window) {
  if (window->monitor) {
    ...
    if (!window->x11.transparent) {
      ...
      XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_BYPASS_COMPOSITOR, XA_CARDINAL, 32, PropModeReplace, (unsigned char*)&value, 1);
    }
  } else {
    if (_glfw.x11.xinerama.available &&
        _glfw.x11.NET_WM_FULLSCREEN_MONITORS) {
...
static unsigned int decodeUTF8(const char** s) {
  ...
  static const unsigned int offsets[] =
      {
          0x00000000u, 0x00003080u, 0x000e2080u, 0x03c82080u, 0xfa082080u, 0x82082080u};
...
static void disableCursor(_GLFWwindow* window) {
  ...
  XGrabPointer(_glfw.x11.display, window->x11.handle, True, ButtonPressMask | ButtonReleaseMask | PointerMotionMask, GrabModeAsync, GrabModeAsync, window->x11.handle, _glfw.x11.hiddenCursorHandle, CurrentTime);
}
...
static GLFWbool createNativeWindow(_GLFWwindow* window,
  ...
  if (_glfw.x11.NET_WM_STATE && !window->monitor) {
    ...
    if (count) {
      XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_STATE, XA_ATOM, 32, PropModeReplace, (unsigned char*)&states, count);
    }
  }
...
int _glfwPlatformWindowHovered(_GLFWwindow* window) {
  ...
  while (w) {
    ...
    if (!XQueryPointer(_glfw.x11.display, w, &root, &w, &rootX, &rootY, &childX, &childY, &mask)) {
      return GLFW_FALSE;
    }
...
void _glfwPlatformSetWindowFloating(_GLFWwindow* window, GLFWbool enabled) {
  ...
  if (_glfwPlatformWindowVisible(window)) {
    ...
    if (enabled) {
      ...
      if (i == count) {
        XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_STATE, XA_ATOM, 32, PropModeAppend, (unsigned char*)&_glfw.x11.NET_WM_STATE_ABOVE, 1);
      }
    } else {
      for (i = 0; i < count; i++) {
        if (states[i] == _glfw.x11.NET_WM_STATE_ABOVE) {
