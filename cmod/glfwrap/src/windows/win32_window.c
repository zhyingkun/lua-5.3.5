...
static void updateFramebufferTransparency(const _GLFWwindow* window) {
  ...
  if (SUCCEEDED(DwmIsCompositionEnabled(&enabled)) && enabled) {
    ...
    exStyle &= ~WS_EX_LAYERED;
    SetWindowLongW(window->win32.handle, GWL_EXSTYLE, exStyle);
    RedrawWindow(window->win32.handle, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME);
  }
}
...
static int createNativeWindow(_GLFWwindow* window,
  ...
  if (window->monitor) {
    ...
    getFullWindowSize(style, exStyle, wndconfig->width, wndconfig->height, &fullWidth, &fullHeight, USER_DEFAULT_SCREEN_DPI);
  }
  ...
  if (!window->monitor) {
    ...
    SetWindowPos(window->win32.handle, NULL, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, SWP_NOACTIVATE | SWP_NOZORDER);
  }
