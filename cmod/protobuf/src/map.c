...
static struct map_ip* _pbcM_ip_new_hash(struct map_kv* table, int size) {
...
static void _inject(struct map_kv* table, struct map_ip* map) {
  if (map->array) {
    int n = 0;
    int i;
...
static void _pbcM_sp_insert_hash(struct map_sp* map, const char* key, size_t hash_full, void* value) {
  if (map->cap > map->size) {
...
static void** _pbcM_sp_query_insert_hash(struct map_sp* map, const char* key, size_t hash_full) {
  size_t hash = hash_full & (map->cap - 1);
  struct _pbcM_sp_slot* slot = &map->slot[hash];
  if (slot->key == NULL) {
    if (map->cap <= map->size)
...
static int _find_first(struct map_sp* map) {
  size_t i;
  for (i = 0; i < map->cap; i++) {
    if (map->slot[i].pointer) {
      return i;
...
static int _find_next(struct map_sp* map, const char* key) {
  size_t hash_full = calc_hash(key);
  size_t hash = hash_full & (map->cap - 1);
