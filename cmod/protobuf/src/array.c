...
void _pbcA_open_heap(pbc_array _array, struct heap* h) {
  struct array* a = (struct array*)_array;
  a->number = 0;
  a->heap = h;
  a->a = (union _pbc_var*)(a + 1);
...
struct pbc_slice* pbc_array_slice(pbc_array _array, int index) {
  struct array* a = (struct array*)_array;
...
void pbc_array_push_slice(pbc_array array, struct pbc_slice* s) {
  pbc_var var;
  var->m = *s;
  _pbcA_push(array, var);
}
