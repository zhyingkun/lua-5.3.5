...
static inline int write_real(int ctype, double v, void* out) {
  switch (ctype) {
    case CTYPE_DOUBLE:
      *(double*)out = v;
      return 0;
  ...
  switch (ptype) {
    case PTYPE_DOUBLE:
      ...
      for (i = 0; i < size; i += 8) {
        ...
        u.i64 = (uint64_t)buffer[i] | (uint64_t)buffer[i + 1] << 8 | (uint64_t)buffer[i + 2] << 16 |
                (uint64_t)buffer[i + 3] << 24 | (uint64_t)buffer[i + 4] << 32 | (uint64_t)buffer[i + 5] << 40 |
                (uint64_t)buffer[i + 6] << 48 | (uint64_t)buffer[i + 7] << 56;
        var->real = u.d;
    ...
    case PTYPE_FLOAT:
      ...
      for (i = 0; i < size; i += 4) {
        ...
        u.i32 = (uint32_t)buffer[i] | (uint32_t)buffer[i + 1] << 8 | (uint32_t)buffer[i + 2] << 16 |
    ...
    case PTYPE_SFIXED32:
      if (size % 4 != 0)
        return -1;
      for (i = 0; i < size; i += 4) {
        var->integer.low = (uint32_t)buffer[i] | (uint32_t)buffer[i + 1] << 8 | (uint32_t)buffer[i + 2] << 16 |
                           (uint32_t)buffer[i + 3] << 24;
        _pbcA_push(array, var);
      }
      return size / 4;
    ...
    case PTYPE_SFIXED64:
      ...
      for (i = 0; i < size; i += 8) {
        var->integer.low = (uint32_t)buffer[i] | (uint32_t)buffer[i + 1] << 8 | (uint32_t)buffer[i + 2] << 16 |
                           (uint32_t)buffer[i + 3] << 24;
        var->integer.hi = (uint32_t)buffer[i + 4] | (uint32_t)buffer[i + 5] << 8 | (uint32_t)buffer[i + 6] << 16 |
...
static inline void _fix32_encode(uint32_t v, uint8_t* buffer) {
  buffer[0] = (uint8_t)v;
...
static int _ctype(const char* ctype) {
  if (ctype[0] != '%')
    return -1;
  switch (ctype[1]) {
    case 'f':
...
static int _ctype_size(const char* ctype) {
  switch (ctype[1]) {
    case 'f':
      return sizeof(float);
    case 'F':
...
static const char* _copy_string(const char* format, char** temp) {
  char* output = *temp;
...
static int _scan_pattern(const char* format, char* temp) {
  int n = 0;
  for (;;) {
...
static int _check_ctype(_field* field, _pattern_field* f) {
  ...
  if (field->type == PTYPE_ENUM) {
    return !(f->ctype == CTYPE_INT8 || f->ctype == CTYPE_INT8 || f->ctype == CTYPE_INT16 || f->ctype == CTYPE_INT32 ||
             f->ctype == CTYPE_INT64);
  }

  return f->ctype == CTYPE_VAR || f->ctype == CTYPE_ARRAY || f->ctype == CTYPE_PACKED || f->ctype == CTYPE_DOUBLE ||
         f->ctype == CTYPE_FLOAT;
}
