...
#define VEC_ABS(src, dst) vec_abs((const Vec*)src, (Vec*)dst)
#define VEC_ABS_(dst) vec_abs((const Vec*)dst, (Vec*)dst)
#define VEC_ADD(src1, src2, dst) vec_add((const Vec*)src1, (const Vec*)src2, (Vec*)dst)
#define VEC_ADD_(dst, src2) vec_add((const Vec*)dst, (const Vec*)src2, (Vec*)dst)
#define VEC_SUBTRACT(src1, src2, dst) vec_subtract((const Vec*)src1, (const Vec*)src2, (Vec*)dst)
#define VEC_SUBTRACT_(dst, src2) vec_subtract(const(Vec*) dst, (const Vec*)src2, (Vec*)dst)
#define VEC_COMPONENT_WISE_PRODUCT(src1, src2, dst) vec_componentWiseProduct((const Vec*)src1, (const Vec*)src2, (Vec*)dst)
#define VEC_COMPONENT_WISE_PRODUCT_(dst, src2) vec_componentWiseProduct((const Vec*)dst, (const Vec*)src2, (Vec*)dst)
#define VEC_SCALE(src, scale, dst) vec_scale((const Vec*)src, scale, (Vec*)dst)
#define VEC_SCALE_(dst, scale) vec_scale((const Vec*)dst, scale, (Vec*)dst)
#define VEC_DOT_PRODUCT(vec1, vec2) vec_dotProduct((const Vec*)vec1, (const Vec*)vec2)
#define VEC_LENGTH_SQUARED(vec) vec_lengthSquared((const Vec*)vec)
#define VEC_LENGTH(vec) vec_length((const Vec*)vec)
#define VEC_DISTANCE_SQUARED(vec1, vec2) vec_distanceSquared((const Vec*)vec1, (const Vec*)vec2)
#define VEC_DISTANCE(vec1, vec2) vec_distance((const Vec*)vec1, (const Vec*)vec2)
#define VEC_NORMALIZE(src, dst) vec_normalize((const Vec*)src, (Vec*)dst)
#define VEC_NORMALIZE_(dst) vec_normalize((const Vec*)dst, (Vec*)dst)
#define VEC_COPY(src, dst) vec_copy((const Vec*)src, (Vec*)dst)
#define VEC_MAX(src1, src2, dst) vec_max((const Vec*)src1, (const Vec*)src2, (Vec*)dst)
#define VEC_MIN(src1, src2, dst) vec_min((const Vec*)src1, (const Vec*)src2, (Vec*)dst)
#define VEC_EQUALS(src1, src2) vec_equals((const Vec*)src1, (const Vec*)src2)
#define VEC_IS_ZERO(vec) vec_isZero((const Vec*)vec)
#define VEC_PROJECTION(src, axis, dst) vec_projection((const Vec*)src, (const Vec*)axis, (Vec*)dst)
#define VEC_PERPENDICULAR(src, axis, dst) vec_perpendicular((const Vec*)src, (const Vec*)axis, (Vec*)dst)
