...
static int NKFONTATLAS_FUNCTION(begin)(lua_State* L) {
  struct nk_font_atlas* atlas = luaL_checkatlas(L, 1);
...
static int NKFONTATLAS_FUNCTION(add)(lua_State* L) {
  ...
  struct nk_font_config* cfg = luaL_checkfontconfig(L, 4);
  struct nk_font* font = nk_font_atlas_add_from_memory(atlas, (void*)memory, size, height, cfg);
...
static int NKFONTATLAS_FUNCTION(add_default)(lua_State* L) {
  ...
  struct nk_font_config* cfg = luaL_checkfontconfig(L, 3);
  struct nk_font* font = nk_font_atlas_add_default(atlas, height, cfg);
  lua_pushlightuserdata(L, (void*)font);
  return 1;
}
static int NKFONTATLAS_FUNCTION(bake)(lua_State* L) {
  struct nk_font_atlas* atlas = luaL_checkatlas(L, 1);
  enum nk_font_atlas_format format = (enum nk_font_atlas_format)luaL_checkinteger(L, 2);
  int width, height;
...
static int NKFONTATLAS_FUNCTION(endatlas)(lua_State* L) {
  ...
  struct nk_draw_null_texture* nullTex = (struct nk_draw_null_texture*)lua_newuserdata(L, sizeof(struct nk_draw_null_texture));
